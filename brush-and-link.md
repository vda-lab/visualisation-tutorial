...
Now that we have created our brushable chart, we may decrease the size of the first chart, so that we can easily display two charts in the screen.
If we now make a second chart that inherits all the properties of the first chart but we only change the x position encoding to `Entity`, we get the two-way brushable and linkable visalisation below.
