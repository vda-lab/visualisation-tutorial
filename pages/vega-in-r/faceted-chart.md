...
For faceting, we can use `facet` inside `$encode()`. The height and width we specify, concern each individual plot. 
If we do not specify the number of columns `columns = 3` all individual charts are concatenated horizontally.
We also see that with the `facet` function, the title of each chart is automatically displayed.
...
{:.exercise}
**Exercise** - What is the problem with the chart you made above and how could you remake it without using `facet`?
